...
```
"""
function PowerModelsDistribution.constraint_mc_switch_state_on_off(pm::LPUBFSwitchModel, nw::Int, i::Int, f_bus::Int, t_bus::Int, f_connections::Vector{Int}, t_connections::Vector{Int}; relax::Bool=false)
...
function PowerModelsDistribution.constraint_mc_bus_voltage_magnitude_sqr_on_off(pm::LPUBFSwitchModel, nw::Int, i::Int, vmin::Vector{<:Real}, vmax::Vector{<:Real})
    ...
    for (idx,t) in [(idx,t) for (idx,t) in enumerate(terminals) if !grounded[idx]]
        if isfinite(vmax[idx])
            JuMP.@constraint(pm.model, w[t] <= vmax[idx]^2*z_block)
        end

        if isfinite(vmin[idx])
            JuMP.@constraint(pm.model, w[t] >= vmin[idx]^2*z_block)
        end
    end
end
...
```
"""
function constraint_mc_transformer_power_yy_on_off(pm::LPUBFSwitchModel, nw::Int, trans_id::Int, f_bus::Int, t_bus::Int, f_idx::Tuple{Int,Int,Int}, t_idx::Tuple{Int,Int,Int}, f_connections::Vector{Int}, t_connections::Vector{Int}, pol::Int, tm_set::Vector{<:Real}, tm_fixed::Vector{Bool}, tm_scale::Real)
...
"""
function constraint_mc_storage_losses_on_off(pm::LPUBFSwitchModel, i::Int; nw::Int=nw_id_default)
    ...
    JuMP.@constraint(pm.model, sum(ps[c] for c in conductors) + (sd - sc) == p_loss * z_block)
    JuMP.@constraint(pm.model, sum(qs[c] for c in conductors) == qsc_zblock + q_loss * z_block)
end
