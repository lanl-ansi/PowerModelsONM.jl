...
function _ref_add_load_blocks!(ref::Dict{Symbol,<:Any}, data_mdl::Union{PMD.EngineeringModel, PMD.MathematicalModel})
    # data = PMD._convert_model_to_dict(data_mdl)
    ref[:blocks] = Dict{Int,Set}(i => block.second for (i,block) in enumerate(sort([sum(map(x->SHA.sha1(ref[:bus][x]["name"]), collect(b)))=>b for b in PMD.calc_connected_components(data_mdl; type="load_blocks", check_enabled=true)]; by=x->x.first)))
    ref[:bus_block_map] = Dict{Int,Int}(bus => b for (b,block) in ref[:blocks] for bus in block)
    ref[:block_branches] = Dict{Int,Set}(b => Set{Int}() for (b,_) in ref[:blocks])
    ref[:block_loads] = Dict{Int,Set}(i => Set{Int}() for (i,_) in ref[:blocks])
    ref[:block_weights] = Dict{Int,Real}(i => 1.0 for (i,_) in ref[:blocks])
...
function _ref_add_options!(
    ...
    ref[:options] = recursive_merge(
        ...
        get(data, "options", Dict{String,Any}()),
    )
end

function _ref_add_options!(
    ref::Dict{Symbol,<:Any},
    data::PMD.MathematicalModel,
)
    return _ref_add_options!(ref, data.data)
end
