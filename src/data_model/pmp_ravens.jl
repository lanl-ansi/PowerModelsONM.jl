function ravens_run_pmp(data, devices)
    ...
    for (name, fault) in data["Fault"]
        ...
        if (bus["bus_type"] != 4)
            ...
            PMP.get_source_graph!(dm, "ConnectivityNode.$(devices["Storage"]["node"])")
            ...
            for (i, bus) in sol["bus"]
                ...
                if bus["name"] == "ConnectivityNode.$(devices["Storage"]["node"])"
                    ...
                    for ph in data_math["bus"][i]["terminals"]
                        ...
                        voltage = Dict(
                            ...
                            "AnalysisResultData.DataValues" => Dict(
                                "AvVoltage.v" => abs(vabc[ph]),
                                "AvVoltage.angle" => angle(vabc[ph])*180/pi,
                        ...
                        current = Dict(
                            ...
                            "AnalysisResultData.phase" => p_name,
                            "AnalysisResultData.DataValues" => Dict(
                                "AvCurrent.i" => abs(iabc[ph]),
                                "AvCurrent.angle" => angle(iabc[ph])*180/pi,
                    ...
                    if haskey(branch, "p_matrix")
                        ...
                        for ph in data_math["bus"][i]["terminals"]
                            ...
                            voltage = Dict(
                                ...
                                "AnalysisResultData.phase" => p_name,
                                "AnalysisResultData.DataValues" => Dict(
                                    "AvVoltage.v" => abs(_v[ph+3]),
                                    "AvVoltage.angle" => angle(_v[ph+3])*180/pi,
                                    "Ravens.cimObjectType" => "AvVoltage",
                                )
                            )
                            ...
                            current = Dict(
                                ...
                                "AnalysisResultData.phase" => p_name,
                                "AnalysisResultData.DataValues" => Dict(
                                    "AvCurrent.i" => abs(iabc[ph+3]),
                                    "AvCurrent.angle" => angle(iabc[ph+3])*180/pi,
                                    "Ravens.cimObjectType" => "AvCurrent",
                                )
                            )
