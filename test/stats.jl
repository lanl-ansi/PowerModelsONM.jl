@testset "test statistical analysis functions" begin
    ...
    @testset "test dispatch stats" begin
        ...
        @test all(isapprox.(args["output_data"]["Powerflow output"][3]["solar"]["pv_mg1b"]["reactive power setpoint (kVar)"], [-1.81965, -1.81965, -1.81965]; atol=1e-1))
    ...
    @testset "test microgrid stats" begin
        ...
        @test all(isapprox.(args["output_data"]["Load served"]["Microgrid load (%)"], [14.2464, 17.9733, 51.9408, 77.8433, 61.1882, 53.4059, 82.2703, 81.8945]; atol=1e-1))
        ...
        @test all(isapprox.(args["output_data"]["Generator profiles"]["Solar DG (kW)"], [0.0, 0.0, 13.9976, 219.997, 27.9965, 10.4952, 0.0, 0.0]; atol=1e0))
